...
def create_new_ekt(aad: bytes) -> bytes:
    ...
    with open_token_session() as session:
        cmk_h = cloudhsm_generate_new_cmk(session)
        try:
            ekt = cloudhsm_derive_kek_and_wrap(session,
                wrapping_key_handle=get_domain_key_handle(),
                secret_handle=cmk_h,
                aad=aad
            )
        finally:
            cloudhsm_delete_object(session, cmk_h)
    return ekt
...
def generate_new_secret(secret_aad: bytes,
    ...
    with open_token_session() as session:
        secret_h = cloudhsm_generate_new_secret(session)
        try:
            cmk_h = cloudhsm_derive_kek_and_unwrap(session,
                wrapping_key_handle=get_domain_key_handle(),
                cipher_text=ekt,
                aad=ekt_aad,
            ...
            try:
                rsa_h = cloudhsm_import_rsa_public_key(session, rsa_pub_key)
                try:
                    aes_wrapped_secret = cloudhsm_derive_kek_and_wrap(session,
                        wrapping_key_handle=cmk_h,
                        secret_handle=secret_h,
                        aad=secret_aad
                    )
                    aes_wrapped_secret_sig = sign(aes_wrapped_secret)
                    rsa_wrapped_secret = cloudhsm_wrap_secret_rsa(session, rsa_h, secret_h)
                    rsa_wrapped_secret_sig = sign(rsa_wrapped_secret)
                    # Cleanup
                finally:
                    cloudhsm_delete_object(session, rsa_h)
            finally:
                cloudhsm_delete_object(session, cmk_h)
        finally:
            cloudhsm_delete_object(session, secret_h)
...
def decrypt_secret(aes_wrapped_secret: bytes, secret_aad: bytes,
    ...
    with open_token_session() as session:
        ...
        try:
            rsa_h = cloudhsm_import_rsa_public_key(session, rsa_pub_key)
            try:
                secret_h = cloudhsm_derive_kek_and_unwrap(session,
                    wrapping_key_handle=cmk_h,
...
def reencrypt_secret(aes_wrapped_secret: bytes,
    ...
    with open_token_session() as session:
        ...
        try:
            ...
            try:
                new_aes_wrapped_secret = cloudhsm_derive_kek_and_wrap(session,
                    wrapping_key_handle=cmk_h,
                    secret_handle=secret_h,
                    aad=new_secret_aad
                )
