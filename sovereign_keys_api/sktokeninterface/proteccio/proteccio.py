...
# CONSTS #
##########
PKCS11_LIB = '/opt/tw_proteccio/lib/libnethsm.so'
...
# HSM interactions #
####################
from skpkcs11 import ProteccioLib, Mechanism, KeyType
_p_lib = ProteccioLib.get_instance(PKCS11_LIB)
_token = None
...
def test_proteccio_connection(pin=None):
    if pin is None:
        pin = _PIN
    if pin is None:
...
def proteccio_set_pin(pin):
    global _PIN
    with _pin_lock:
        test_proteccio_connection(pin)
        _PIN = pin
...
def _derive_kek(session, key_handle, nonce):
    return session.derive_master_key(
        ...
        derive_key_mech=Mechanism.AES_ECB_ENCRYPT_DATA,
        nonce=nonce,
        key_label='tmp_derived_secret',
        key_type=KeyType.AES,
        key_size_in_bits=256,
        token=False,        # session key (wiped when session is closed)
        private=True,       # It is not visible if not logged-in
        sensitive=True,     # It cannot be exported in cleartext
...
def proteccio_derive_kek_and_wrap(session, wrapping_key_handle, secret_handle, aad):
    ...
    # Generate a random 96-bits IV and 256-bits Nonce
    logger.debug('HSM: Generate IV (96 bits) and Nounce (256bits)')
    r = session.generate_random(size=44)
    iv = r[:12]
    nonce = r[12:]
...
def proteccio_derive_kek_and_unwrap(session, wrapping_key_handle, cipher_text, aad, target_key_type, extractable):
...
# Generate new CMK
def proteccio_generate_new_cmk(session):
...
# Import RSA pub key
def proteccio_import_rsa_public_key(session, rsa_pub_key):
...
def proteccio_generate_new_secret(session):
...
def proteccio_wrap_secret_rsa(session, pub_key_handle, secret_handle):
